...
# @Last Modified time: 2019-04-29 21:59:59
...
if os.path.isfile(X_filename) and os.path.isfile(y_filename):
        ...
        for json_file in zip_file.infolist():
            ...
            for train in df.itertuples():
                ...
                row = {
                    ...
                    'type': train_type,
                    'howLateOnAverage': how_late_on_average,
    ...
    X_df = pd.DataFrame(X).to_csv(X_filename)
    y_df = pd.DataFrame(y, columns=['averageDelayInMinutes']).to_csv(y_filename)
